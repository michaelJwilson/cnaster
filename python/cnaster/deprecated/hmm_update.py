...
def update_emission_params_nb_sitewise_uniqvalues(
    ...
    if fix_NB_dispersion:
        ...
        if not shared_NB_dispersion:
            logger.info(
                "Updating (phasing) NB emission parameters with free dispersion."
            )
...
def update_emission_params_nb_sitewise_uniqvalues_mix(
    ...
    if fix_NB_dispersion:
        logger.info(
            "Updating (phasing, tumor mix) NB emission parameters with fixed dispersion."
        )
        ...
        if not shared_NB_dispersion:
            logger.info(
                "Updating (phasing, tumor mix) NB emission parameters with free dispersion."
            )
...
def update_emission_params_bb_sitewise_uniqvalues(
    ...
    if fix_BB_dispersion:
        ...
        if not shared_BB_dispersion:
            logger.info(
                "Updating (phasing) BAF emission parameters with free dispersion."
            )
...
def update_emission_params_bb_nophasing_uniqvalues(
    ...
    if fix_BB_dispersion:
        logger.info(
            "Updating (no phasing) BB emission parameters with fixed dispersion."
        )
        ...
        if not shared_BB_dispersion:
            logger.info(
                "Updating (no phasing) BB emission parameters with free dispersion."
            )
...
def update_emission_params_bb_sitewise_uniqvalues_mix(
    ...
    if fix_BB_dispersion:
        logger.info(
            "Updating (phasing, tumor mix) BAF emission parameters with fixed dispersion."
        )
        ...
        if not shared_BB_dispersion:
            logger.info(
                "Updating (phasing, tumor mix) BAF emission parameters with free dispersion."
            )
...
def update_emission_params_bb_nophasing_uniqvalues_mix(
    ...
    # NB unique_values is a list of length n_spots, each is a 2D array of shape (n_unique_obs, 2) for (a-allele counts, total counts)
    ...
    for s in np.arange(len(unique_values)):
        # NB non-zero total counts for this spot.
        idx_nonzero = np.where(unique_values[s][:, 1] > 0)[0]

        # NB we tile the obs. data across n_states for efficient em-cost evaluation.
        this_y = np.tile(unique_values[s][idx_nonzero, 0], n_states)
        this_exposure = np.tile(unique_values[s][idx_nonzero, 1], n_states)
    ...
    model = Weighted_BetaBinom_mix(
        ...
        fixed_dispersion=fix_BB_dispersion,
        shared_dispersion=shared_BB_dispersion,
    )

...
def multislice_adjacency(
    ...
    if lattice_type is not None:
        ...
        if l_type in lattice_map:
            ...
            logger.info(f"Lattice type '{l_type}' detected. Overriding to n_nearest={n_nearest}.")
        else:
            logger.warning(f"Unknown lattice_type '{lattice_type}'. Falling back to n_nearest={n_nearest}.")
    else:
    ...
    for s in range(num_slices):
        ...
        slice_adj = scipy.sparse.csr_matrix(
            (data, (rows, cols)), 
            shape=(n_slice_spots, n_slice_spots)
        )

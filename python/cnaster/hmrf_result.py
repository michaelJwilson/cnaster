...
class HMRFResult:
    ...
    new_log_mu: np.ndarray       # shape (n_states, n_clones) or similar depending on clone stacking
    ...
    log_gamma: np.ndarray        # shape (n_states, n_obs_stack) - The log posterior probability of each state
    pred_cnv: np.ndarray         # shape (n_obs_stack, ) - The MAP copy-number state estimation

    # Spatial clone assignments for spots
    prev_assignment: np.ndarray  # shape (n_spots, ) - Clone assignment prior to the final iteration
    new_assignment: np.ndarray   # shape (n_spots, ) - Final clone assignment after HMRF smoothing
